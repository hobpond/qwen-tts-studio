...
* **Audiobook layout correction**: Replaced the sparse two-row KTV surface in `scripts/render-audiobook-video.py` with a manifest-driven seven-row reading window. The active line is centered, three sung context lines remain above it, and three blue read-ahead lines remain below it; `--lines-on-screen` supports odd row counts from 3 through 11. The pixel proof now measures the longest active-line sweep so dense static context cannot mask the advancing highlight. A 90-second proof passed stream, pixel, and visual inspection before the full render.
* **Audiobook render throughput**: Added line-aligned parallel video-only segments with a two-worker/600-second default, lossless concat-demuxer joining, and one final AAC encode of the source WAV. The segmented 90-second proof completed in 20.4 seconds, passed stream and pixel verification, and reports its segment boundaries and join/mux policy.
* **Resumable streamable delivery**: Added `--streamable`/`-Streamable`, which caches and validates one AAC/M4A encoding, assembles a fragmented MP4 by stream-copy, writes an HLS fMP4/VOD package, and fingerprints reusable video segments. A 90-second proof passed stream/pixel/HLS checks; a second pass reused the audio cache and all four video segments.
* **Pipeline orchestration foundation**: Added a platform-neutral audiobook command/event/snapshot contract with revision/lease guards, explicit chunk readiness states, and stale-event reduction, plus a desktop mutex/reducer adapter and a Compose-safe UI binding. Focused subagent tests passed for the contract, desktop adapter, and binding; wiring concrete StudioViewModel actions into the shared executor remains the next integration slice.

## 2026-08-17

* **Audiobook video instrumentation**: Added a manifest-driven Windows/Python exporter that adapts the KTV renderer's ASS `\\kf` two-line sweep to validated Chinese/Vivian TTS. It muxes the existing combined WAV, reports the honest `uniform-character-within-chunk` timing estimate, writes ASS/report sidecars, and fails closed unless stream and pixel-level sweep checks pass. A real 90-second proof passed at 1280x720 with one AAC audio stream and rising sung-pixel counts.
