...
| Empty native submodule | Run `git submodule update --init --recursive`, then rebuild. The CMake wrapper adds `qwen3-tts-cpp` as a subdirectory, so an empty checkout cannot configure the backend. |
