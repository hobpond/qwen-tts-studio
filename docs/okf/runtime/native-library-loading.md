...
- [`composeApp/build.gradle.kts`](../../../composeApp/build.gradle.kts) forwards the optional `-PnativeRuntimeRoot=PATH` project property to the desktop application's `jna.library.path`, allowing an instrumented run to select a known runtime directory ahead of a stale working-directory copy.
